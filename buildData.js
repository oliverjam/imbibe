...
const containsIce = method => {
  const str = method.toLowerCase();
  const icePhrases = ['filled with ice cubes', 'over a large cup of ice', 'over ice', 'add ice'];
  return icePhrases.some(phrase => str.includes(phrase));
}
...
/*
 type Drinks = Array<
  {|
    ...
    id: string,
    name: string,
    alcoholic: 'alcoholic' | 'non-alcoholic' | 'optional',
    glass: string,
    image: string,
    method: string,
    hasIce: boolean
 ...
 >
*/
const drinks = source.map(drink => Object.entries(drink).reduce((acc, [key, value]) => {
  if (!value || !(value.trim())) return acc;
  if (key === 'idDrink') return { ...acc, id: value }
  if (key === 'strDrink') return { ...acc, name: value }
  if (key === 'strDrinkThumb') return { ...acc, image: value }
  if (key === 'strGlass') return { ...acc, glass: value }
  if (key === 'strInstructions') return { ...acc, method: value, hasIce: containsIce(value) }
  if (key === 'strAlcoholic') return { ...acc, alcoholic: value === 'Alcoholic' ? 'alcoholic' : value === 'Non Alcoholic' ? 'non-alcoholic' : 'optional' };
  if (key.includes('Ingredient')) return { ...acc, ingredients: [...acc.ingredients, value.toLowerCase()] };
  if (key.includes('Measure')) return { ...acc, measures: [...acc.measures, value.toLowerCase()] };
  return acc;
}, { ingredients: [], measures: [] }))

// const alcoholic = drinks.filter(drink => drink.strAlcoholic === 'Alcoholic');
// const ingredients = Array.from(new Set(drinks.map(drink => drink.ingredients).flat()));

// console.log(drinks.slice(0, 45));

writeFile('./src/data/drinks.js', `export default ${JSON.stringify(drinks)}`, error =>
  error
  ? console.error('⚠️ Error building drink data', error)
  : console.log('🍸 Drink data built successfully'));